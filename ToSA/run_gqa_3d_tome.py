...
if __name__ == "__main__":
    ...
    # if osp.exists(f'gqa/results/gqa_3d_tome_r_{token_merging_ratio}.json'):
    #     with open(f'gqa/results/gqa_3d_tome_r_{token_merging_ratio}.json') as f:
    #         results = json.load(f)
    
    answered_count = len([r for r in results if 'answer' in r])
    print('Answered count:', answered_count)

    for i, img_path in zip(pbar, img_path_to_qa):
        for question, ans in img_path_to_qa[img_path]:
            cur_count += 1
            if cur_count <= answered_count:
                continue
    ...
    if alpha_style == 'uniform':
        with open(f'gqa/results/gqa_3d_tome_r_{token_merging_ratio}_{alpha_style}_full_spatial.json', 'w') as f:
            json.dump(results, f, indent=4)
    else:
        with open(f'gqa/results/gqa_3d_tome_r_{token_merging_ratio}_{alpha_style}.json', 'w') as f:
            json.dump(results, f, indent=4)