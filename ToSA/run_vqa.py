...
class llava_chatbot:
    ...
    def generate_response(self, question, conv_template="qwen_1_5"):
        question = f"{DEFAULT_IMAGE_TOKEN}" + f"'{question}' Answer the question using a single word. \n\n"
...
if __name__ == "__main__":
    ...
    with open('vqa/count_qa.json', 'r') as f:
        data = json.load(f)
        
    img_path_to_qa = defaultdict(list)
    
    for d in data.values():
        img_path = os.path.join('vqav2/val2014/COCO_val2014_{:012d}.jpg'.format(d['image_id']))
        question = d['question']
        answer = d['answer']
    ...
    with open(f'vqav2/results/vqav2_og.json', 'w') as f:
